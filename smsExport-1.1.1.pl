...
if ($^O =~ /darwin/i) {
	$smsExportsDirectory = "$ENV{'HOME'}/smsExports";
}
...
## File to write to.
my $messageWithJoinCSVfile = "$outputDirectory/$Device_Name-sms.csv";
...
message.ROWID, 
datetime(message.date / 1000000000 + 978307201,'unixepoch') as date, 
message.handle_id, 
handle.id,
...
my $messageCSVwithContacts = "$outputDirectory/$Device_Name-smsWithContacts.csv";
